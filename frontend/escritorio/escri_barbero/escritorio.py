        ...
        if r.status_code == 200:
            for c in r.json():
                tree.insert("", "end", values=(
                    c["id"], c["nombre"], c["correo"], c.get("especialidad", ""), c["telefono"]
                ))
    except Exception as e:
        messagebox.showerror("Error", str(e))

def crear_barbero():
    dialogo_barbero("Crear nuevo barbero")

def editar_barbero():
    seleccionado = tree.focus()
    if not seleccionado:
        messagebox.showwarning("Seleccionar", "Seleccione un barbero.")
        return
    valores = tree.item(seleccionado, "values")
    dialogo_barbero("Editar barbero", valores)
...
def eliminar_barbero():
    ...
    if messagebox.askyesno("Eliminar", "¿Seguro que desea eliminar este barbero?"):
        try:
            r = requests.delete(API + ENDPOINTS["delete"].replace("{id}", str(id_com)))
            if r.status_code == 200:
                recargar_datos()
                messagebox.showinfo("Éxito", "barbero eliminado.")
            else:
                messagebox.showerror("Error", r.text)
        except Exception as e:
            messagebox.showerror("Error", str(e))

def dialogo_barbero(titulo, datos=None):
    ...
    especialidad.grid(row=2, column=1)
    ...
    if datos:
        id_com, nombre_val, correo_val, especialidad_val, telefono_val = datos
        nombre.insert(0, nombre_val)
        correo.insert(0, correo_val)
        especialidad.insert(0, especialidad_val)
...
# Ventana principal
root = tk.Tk()
root.title("Gestión de barberos")

# Tabla
cols = ("ID", "Nombre", "correo", "especialidad", "telefono")
...
tk.Button(botonera, text="Nuevo", command=crear_barbero).pack(side="left", padx=5)
tk.Button(botonera, text="Editar", command=editar_barbero).pack(side="left", padx=5)
tk.Button(botonera, text="Eliminar", command=eliminar_barbero).pack(side="left", padx=5)
